...
void Modbus::addReg(word address, word value) {
  // // Serial.println("\n==> addReg addr: "+String(address)+" value: "+String(value));
    TRegister *newreg;
...
word Modbus::Reg(word address) {
    ...
    if(reg) {
        // Serial.println("\n==> ReadReg addr: "+String(address)+" value: "+String(reg->value));
        return(reg->value);
    }
    else {
        // Serial.println("\n==> ReadReg addr: "+String(address)+" NOT FOUND");
        return(0);
    }
}
...
void Modbus::receivePDU(byte* frame) {
    ...
    switch (fcode) {        // Serial.print(" CODE:"+String(fcode));

        case MB_FC_WRITE_REG:
            //field1 = reg, field2 = value
            // Serial.print(" writeSingleRegister ");
            this->writeSingleRegister(field1, field2);
        break;

        case MB_FC_READ_REGS:
            //field1 = startreg, field2 = numregs
            // Serial.print(" readRegisters: "+String(field1)+" "+String(field2)+" ");
            this->readRegisters(field1, field2);
        break;

        case MB_FC_WRITE_REGS:
            //field1 = startreg, field2 = status
            // Serial.print(" writeMultipleRegisters ");
        ...
        case MB_FC_WRITE_COILS:
            ...
            // Serial.print(" writeMultipleCoils ");
...
void Modbus::readRegisters(word startreg, word numregs) {
  // Serial.println("\n==> ReadReg started: "+String(startreg)+" numregs: "+String(numregs));
    //Check value (numregs)
	...
	while(numregs--) {
      ...
      // Serial.println("\n--> read: numregs: "+String(numregs)+" val: "+String(val));
